...
def config():
    ...
    #weights = '/home/kiapi/Documents/MRCNet/ourdata/val_seq_00/models/checkpoint_r15.00_t0.30_e75_0.006.tar'
    weights = None
    rescale_rot = 1.0
    rescale_transl = 2.0
    precision = "O0"
...
def main(_config, _run, seed):
    ...
    val_sequence_str = _config['val_sequence']
    if isinstance(_config['val_sequence'], list):
        val_sequence_str = '_'.join(_config['val_sequence'])  # 리스트를 "_"로 연결된 문자열로 변환

    # 모델 저장 경로 생성
    model_savepath = os.path.join(_config['checkpoints'], 'val_seq_' + val_sequence_str, 'models')

    if not os.path.exists(model_savepath):
        os.makedirs(model_savepath)
    #log_savepath = os.path.join(_config['checkpoints'], 'val_seq_' + _config['val_sequence'], 'log')
    log_savepath = os.path.join(_config['checkpoints'], 'val_seq_' + val_sequence_str, 'log')
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            #resize
            rgb_input = F.interpolate(rgb_input, size=[256, 1024], mode="bilinear")
            lidar_input = F.interpolate(lidar_input, size=[256, 1024], mode="bilinear")
