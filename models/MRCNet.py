...
from models.conformer import Conformer, ConvTransBlock
...
class MRCNet(nn.Module):
    ...
    def __init__(self, image_size, use_feat_from=1, md=4, use_reflectance=False, dropout=0.0,
        ...
        self.toplayer_jy = nn.Conv2d(1024, 64, 1, 1, 0)
        #self.toplayer_jy_img = nn.Conv2d(1024, 64, 1, 1, 0)
        ...
        self.latlayer1_jy = nn.Conv2d(1024, 64, 1, 1, 0)
        #self.latlayer1_jy_img = nn.Conv2d(1024, 64, 1, 1, 0)
        self.latlayer2 = nn.Conv2d(128, 64, 1, 1, 0)
        self.latlayer2_jy = nn.Conv2d(512, 64, 1, 1, 0)
        self.latlayer3_jy = nn.Conv2d(256, 64, 1, 1, 0)
        #self.latlayer2_jy_img = nn.Conv2d(512, 64, 1, 1, 0)
        self.latlayer3 = nn.Conv2d(64, 64, 1, 1, 0)

        #conformer 1# 
        # self.conformer_rgb = Conformer(patch_size=16, channel_ratio=6, embed_dim=576, depth=12,
        #               num_heads=9, mlp_ratio=4, qkv_bias=True)
        # self.conformer_lidar = Conformer(patch_size=16, in_chans=1, channel_ratio=6, embed_dim=576, depth=12,
        #               num_heads=9, mlp_ratio=4, qkv_bias=True)
        #conformer 2# 
        self.conformer_rgb = Conformer(patch_size=16, channel_ratio=4, embed_dim=768, depth=12,
                      num_heads=12, mlp_ratio=4, qkv_bias=True)
        self.conformer_lidar = Conformer(patch_size=16, in_chans=1, channel_ratio=4, embed_dim=768, depth=12,
                      num_heads=12, mlp_ratio=4, qkv_bias=True)
        
        self.cross_attn_img_to_lidar = nn.MultiheadAttention(embed_dim=64, num_heads=8, batch_first=True)
        self.cross_attn_lidar_to_img = nn.MultiheadAttention(embed_dim=64, num_heads=8, batch_first=True)

        #self.cross_attn_img_to_lidar = nn.MultiheadAttention(embed_dim=768, num_heads=8, batch_first=True)
        #self.cross_attn_lidar_to_img = nn.MultiheadAttention(embed_dim=768, num_heads=8, batch_first=True)

        #self.cross_attn_img_to_lidar = nn.MultiheadAttention(embed_dim=768, num_heads=8, batch_first=True)
        #self.cross_attn_lidar_to_img = nn.MultiheadAttention(embed_dim=768, num_heads=8, batch_first=True)

        # checkpoint_jy = torch.hub.load_state_dict_from_url(
        #     url="https://dl.fbaipublicfiles.com/deit/deit_base_patch16_224-b5f2ef4d.pth",
        #     map_location="cpu", check_hash=True
        # )
        # #pretrained_path = "https://dl.fbaipublicfiles.com/deit/deit_base_patch16_224-b5f2ef4d.pth"
        # #checkpoint = torch.load(pretrained_path, map_location='cpu')
        # state_dict = checkpoint_jy['model']

        # # classification head key 제거      
        # filtered_state_dict = {k: v for k, v in state_dict.items() if 'head' not in k}

        # self.conformer_rgb.load_state_dict(filtered_state_dict, strict=False)
        # self.conformer_lidar.load_state_dict(filtered_state_dict, strict=False)
        #self.conformer_rgb.load_state_dict(checkpoint_jy["model"], strict=False)
        #self.conformer_lidar.load_state_dict(checkpoint_jy["model"], strict=False)
    ...
    dilation_patch=1)
        ...
        self.fc1_trasl_jy = nn.Linear(135664, 64)   # 256 512  
        self.fc1_rot_jy = nn.Linear(135664, 64)        
        self.fc115_trasl_jy=nn.Linear(8192, 128)              
        self.fc114_trasl_jy=nn.Linear(16384, 64)        
        self.fc113_trasl_jy=nn.Linear(32768, 32)
        self.fc112_trasl_jy=nn.Linear(65536, 16)

        # self.fc115_trasl_jy=nn.Linear(3136, 128)       # 224 448 
        # self.fc114_trasl_jy=nn.Linear(6272, 64)
        # self.fc113_trasl_jy=nn.Linear(12544, 32)
        # self.fc1_trasl_jy = nn.Linear(52066, 64)
        # self.fc1_rot_jy = nn.Linear(52066, 64)
    ...
    def forward(self, rgb, lidar):
        ...
        if self.pretrained_encoder:
            ...
            rgb_feat = self.conformer_rgb(rgb)
            c13 = rgb_feat['features']['stage2']
            c14 = rgb_feat['features']['stage3']
            c15 = rgb_feat['features']['stage4']
            c16 = rgb_feat['features']['stage5']

            lidar_feat = self.conformer_lidar(lidar)
            c23 = lidar_feat['features']['stage2']
            c24 = lidar_feat['features']['stage3']
            c25 = lidar_feat['features']['stage4']
            c26 = lidar_feat['features']['stage5']
        ...
        p5 = self.toplayer_jy(c26)
        p4 = self._upsample_add(p5, self.latlayer1_jy(c25))
        p3 = self._upsample_add(p4, self.latlayer2_jy(c24))
        p2 = self._upsample_add(p3, self.latlayer3_jy(c23))
        ...
        p15 = self.toplayer_jy(c16)
        p14 = self._upsample_add(p15, self.latlayer1_jy(c15))
        p13 = self._upsample_add(p14, self.latlayer2_jy(c14))
        p12 = self._upsample_add(p13, self.latlayer3_jy(c13))

        p14 = self.smooth1(p14)
        p13 = self.smooth2(p13)
        p12 = self.smooth3(p12)

        #p15 = F.interpolate(p15, size=p5.shape[-2:], mode='bilinear', align_corners=False)
        #p5 = F.interpolate(p5, size=p15.shape[-2:], mode='bilinear', align_corners=False)

        B, C, H, W = p5.shape
        p5_reshaped = p5.view(B, C, -1).permute(2, 0, 1)
        p15_reshaped = p15.view(B, C, -1).permute(2, 0, 1)
        p5, _ = self.cross_attn_img_to_lidar(query=p5_reshaped,key=p15_reshaped,value=p15_reshaped)
        p15, _ = self.cross_attn_lidar_to_img(query=p15_reshaped,key=p5_reshaped,value=p5_reshaped)
        p5 = p5.permute(1, 2, 0).view(B, C, H, W)
        p15 = p15.permute(1, 2, 0).view(B, C, H, W)

        corr5 = self.corr(p5.contiguous(), p15.contiguous())
        b,ph,pw,h,w=corr5.size()
        corr5=corr5.contiguous().view(b,ph*pw,h,w)/c16.size(1)
        ...
        x115=self.leakyRELU(self.fc115_trasl_jy(x115))

        #p14 = F.interpolate(p14, size=p4.shape[-2:], mode='bilinear', align_corners=False)
        #p4 = F.interpolate(p4, size=p14.shape[-2:], mode='bilinear', align_corners=False)

        B, C, H, W = p4.shape
        p4_reshaped = p4.view(B, C, -1).permute(2, 0, 1)
        p14_reshaped = p14.view(B, C, -1).permute(2, 0, 1)
        p4, _ = self.cross_attn_img_to_lidar(query=p4_reshaped,key=p14_reshaped,value=p14_reshaped)
        p14, _ = self.cross_attn_lidar_to_img(query=p14_reshaped,key=p4_reshaped,value=p4_reshaped)
        p4 = p4.permute(1, 2, 0).view(B, C, H, W)
        p14 = p14.permute(1, 2, 0).view(B, C, H, W)

        corr4 = self.corr(p4.contiguous(), p14.contiguous())
        b,ph,pw,h,w=corr4.size()
        corr4=corr4.contiguous().view(b,ph*pw,h,w)/c16.size(1)
        # print("corr4.shape (before view):", corr4.shape)
        # print("expected b, ph, pw, h, w:", b, ph, pw, h, w)
        # print("total elements:", corr4.numel(), "expected:", b * ph * pw * h * w)      
        ...
        x114=self.leakyRELU(self.fc114_trasl_jy(x114))

        #p13 = F.interpolate(p13, size=p3.shape[-2:], mode='bilinear', align_corners=False)
        #p3 = F.interpolate(p3, size=p13.shape[-2:], mode='bilinear', align_corners=False)
        # print(p3.size())
        # print(p13.size())
        B, C, H, W = p3.shape
        p3_reshaped = p3.view(B, C, -1).permute(2, 0, 1)
        p13_reshaped = p13.view(B, C, -1).permute(2, 0, 1)
        p3, _ = self.cross_attn_img_to_lidar(query=p3_reshaped,key=p13_reshaped,value=p13_reshaped)
        p13, _ = self.cross_attn_lidar_to_img(query=p13_reshaped,key=p3_reshaped,value=p3_reshaped)
        p3 = p3.permute(1, 2, 0).view(B, C, H, W)
        p13 = p13.permute(1, 2, 0).view(B, C, H, W)

        corr3 = self.corr(p3.contiguous(), p13.contiguous())
        b,ph,pw,h,w=corr3.size()
        corr3=corr3.contiguous().view(b,ph*pw,h,w)/c16.size(1)
        ...
        x113=self.leakyRELU(self.fc113_trasl_jy(x113))


        B, C, H, W = p2.shape
        p2_reshaped = p2.view(B, C, -1).permute(2, 0, 1)
        p12_reshaped = p12.view(B, C, -1).permute(2, 0, 1)
        p2, _ = self.cross_attn_img_to_lidar(query=p2_reshaped,key=p12_reshaped,value=p12_reshaped)
        p12, _ = self.cross_attn_lidar_to_img(query=p12_reshaped,key=p2_reshaped,value=p2_reshaped)
        p2 = p2.permute(1, 2, 0).view(B, C, H, W)
        p12 = p12.permute(1, 2, 0).view(B, C, H, W)

        corr2 = self.corr(p2.contiguous(), p12.contiguous())
        b,ph,pw,h,w=corr2.size()
        corr2=corr2.contiguous().view(b,ph*pw,h,w)/c16.size(1)

        x112=self.leakyRELU(corr2)
        x112=self.conv10_0(x112)#
        x112=self.conv10_1(x112)#
        x112=self.conv10_2(x112)#
        x112 = x112.view(x112.shape[0], -1)
        x112 = self.dropout(x112)
        x112 = self.leakyRELU(x112)
        x112=self.leakyRELU(self.fc112_trasl_jy(x112))
        ...
        transl = self.leakyRELU(self.fc1_trasl_jy(x))
        rot = self.leakyRELU(self.fc1_rot_jy(x))
